                ...
                try {
                    ...
                    for (WatchEvent<?> event : key.pollEvents()) {
                        myEventTable.addEvent(new FileEvent(event.context().toString(),
                                ...
                                eventTypeFormatter(event.kind()), getExtension(event).toString(), createDateString()));
                    }
                    key.reset(); // Reset the key to receive further events /* throws when program stopped while
                                 // take() method is waiting for new event */
